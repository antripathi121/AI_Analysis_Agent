...
def selected_columns_for_period(week_cols, start_date, end_date):
    ...
    for col, week_start in week_cols.items():
...
def sum_section_for_segments(sheet, section_name, segments):
    ...
    for segment in segments:
        cols = selected_columns_for_period(
            week_cols,
            segment["start"],
            segment["end"]
        )
        ...
        segment_total = 0
        segment_store_count = 0

        for row in range(data_start, data_end):
            store_value = sheet.iat[row, store_col]

            if store_is_allowed(store_value, segment["stores"]):
                segment_store_count += 1
                store_names_used.add(clean_text(store_value))

                row_values = sheet.iloc[row, cols]
                row_values = pd.to_numeric(row_values, errors="coerce").fillna(0)
                segment_total += float(row_values.sum())

        grand_total += segment_total
        total_weeks_found += len(cols)
        ...
        segment_details.append({
            ...
            "Store Rows Used": segment_store_count,
            "Weeks Found": len(cols),
            "Subtotal": segment_total
        })

    return {
        "total": float(grand_total),
        "weeks_found": total_weeks_found,
...
st.caption("Offline campaign analysis with period-specific rollout store coverage")
    ...
    if st.button("🚀 Analyze Campaign", use_container_width=True):
        try:
            ...
            for name in periods.keys():
                unit_results[name] = sum_section_for_segments(
                    sales_sheet,
                    "Unit Sales by Store",
                    period_coverage_segments[name]
                )
                shopper_results[name] = sum_section_for_segments(
                    sales_sheet,
            ...
            st.download_button(
                ...
                data=excel_output,
                file_name=f"{sales_cpg}_Campaign_Analysis_Report.xlsx",
